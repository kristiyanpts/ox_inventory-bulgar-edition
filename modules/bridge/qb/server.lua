...
RegisterNetEvent("ox_inventory:server:openSafeInventory", function()
	local Player = QBCore.Functions.GetPlayer(source)
	local citizenid = Player.PlayerData.citizenid
	exports.ox_inventory:RegisterStash("PersonalStash-"..citizenid, "Safe Inventory", 4, 10000, nil, false)
end)

lib.callback.register('ox_inventory:createBackpackSecret', function(source)
	local backpack, secret
	local Player = QBCore.Functions.GetPlayer(source)

	backpack = Inventory(Player.PlayerData.citizenid .. "-backpack") and Inventory(Player.PlayerData.citizenid .. "-backpack") or Inventory.Create(Player.PlayerData.citizenid .. "-backpack", "Backpack", "backpack", 25, 250000, 250000, true, nil, nil, "fa-solid fa-suitcase")
	secret = Inventory(Player.PlayerData.citizenid .. "-secret") and Inventory(Player.PlayerData.citizenid .. "-secret") or Inventory.Create(Player.PlayerData.citizenid .. "-secret", "Safe Inventory", "secret", 5, 5000, 5000, true, nil, nil, "fa-solid fa-user-secret")

	return backpack, secret
end)

lib.callback.register('ox_inventory:getPlayerBackpack', function(source, targetId)
	local backpack
	local Player = QBCore.Functions.GetPlayer(source)
	local PlayerTarget = QBCore.Functions.GetPlayer(targetId)

	print(PlayerTarget.PlayerData.citizenid)

	backpack = Inventory(PlayerTarget.PlayerData.citizenid .. "-backpack") and Inventory(PlayerTarget.PlayerData.citizenid .. "-backpack") or Inventory.Create(PlayerTarget.PlayerData.citizenid .. "-backpack", "Backpack", "backpack", 25, 250000, 250000, true, nil, nil, "fa-solid fa-suitcase")

	backpack.label = "Backpack - " .. PlayerTarget.PlayerData.citizenid
...
lib.callback.register('ox_inventory:getPlayerCitizenId', function(source, targetId, otherPlayer)
	local backpack, backpackTarget

	if otherPlayer then backpackTarget = targetId else backpackTarget = source end
	-- if targetId ~= nil then backpackTarget = targetId else backpackTarget = source end

	local PlayerTarget = QBCore.Functions.GetPlayer(backpackTarget)

	if PlayerTarget == nil then return end

	return PlayerTarget.PlayerData.citizenid
end)

lib.callback.register('ox_inventory:getPlayerSecret', function(source, targetId)
	local backpack
	local Player = QBCore.Functions.GetPlayer(source)
	local PlayerTarget = QBCore.Functions.GetPlayer(targetId)

	backpack = Inventory(PlayerTarget.PlayerData.citizenid .. "-secret") or Inventory.Create(PlayerTarget.PlayerData.citizenid .. "-secret", "Safe Inventory", "secret", 5, 5000, 5000, true, nil, nil, "fa-solid fa-user-secret")

	backpack.label = "Safe Inventory - " .. PlayerTarget.PlayerData.citizenid
